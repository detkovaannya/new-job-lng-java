# Выполнено тестовое задание для компании Уно-Софт
...
# Запуск проекта
1. Скачать проект на локальный компьютер, в папке проекта через консоль выполнить следующие команды:
2. `mvn clean install`
3. `java -jar -Xmx1G target\GroupingTool-1.0.jar <output-file>.txt`
